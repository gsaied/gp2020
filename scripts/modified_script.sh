...
read layername ; 
	find *.mem | grep -v file| grep -v img | grep -v bias | grep -v weights |  while read LINE ; do rm -rf $LINE ; done
...
if [ -z "$file" ] ; then
       	touch file_"$layername"_1_2.mem 
       	 file="file_"$layername"_1_2.mem" 
	echo "
...
if [[ $count -eq $limit ]] ; then
	touch file_"$layername"_"$file_count"_2.mem;
	echo "
  	(* rom_style=\"{distributed}\" *)
	reg [WIDTH-1:0] rom_"$file_count" [0:2**ADDR-1] ;" >> decl.sv;
	#echo -e "\t rom_out[$(($file_count-1))] <= rom_"$file_count"[address];" >> always.sv
	file=file_"$layername"_"$file_count"_2.mem;
